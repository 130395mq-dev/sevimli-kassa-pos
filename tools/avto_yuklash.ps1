...
try {
    ...
    foreach ($r in $repos) {
        ...
        if ($newest -and ((Get-Date) - $newest.LastWriteTime).TotalSeconds -lt $QUIET) {
            Log "$($r.name): o'zgarish bor, yozish tugashini kutyapman ($($newest.Name))"
            continue
        }
        ...
        if ($LASTEXITCODE -ne 0) {
            Log "$($r.name): push bo'lmadi ($out) - force"
            $out = & $git push --force origin main 2>&1
        }
        if ($LASTEXITCODE -eq 0) { Log "$($r.name): GitHub'ga ketdi - $msg" }
        else { Log "$($r.name): XATO push: $out" }
    }
} catch {
